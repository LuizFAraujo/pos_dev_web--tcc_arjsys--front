...
export function NumeroSeriePedidoSearchDialog({
  open,
  onOpenChange,
  pedidos,
  onSelect,
      ...
      <DialogContent className="sm:max-w-5xl">
        ...
        <div className="h-84 overflow-x-auto overflow-y-hidden">
          <DataGrid<PedidoVenda>
            tabId="ns-pedido-search-dialog"
            storageId="ns-pedido-search-dialog"
            columns={columns}
            data={filteredData}
            onSelect={(item) => setSelectedItem(item)}
            onActivate={handleActivate}
            activateOnDoubleClick
            emptyTitle="Nenhum pedido em Aguardando NS"
            emptyDescription="Crie um Pedido de Venda Pré-Venda primeiro."
          />
        </div>
