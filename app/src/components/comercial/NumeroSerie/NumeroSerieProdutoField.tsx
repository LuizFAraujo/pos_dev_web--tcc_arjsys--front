...
import { NumeroSerieProdutoSearchDialog } from './NumeroSerieProdutoSearchDialog';
...
export function NumeroSerieProdutoField({
  id = 'produtoId',
  label = 'Código BOM',
  value,
  displayCodigo,
  ...
  const matchPorCodigo = useCallback(
    (term: string): Produto | null => {
      ...
      if (!t) return null;
      return produtos.find((p) => (p.codigo ?? '').toLowerCase() === t) ?? null;
    },
    [produtos],
  );
  ...
  return (
    <>
      ...
      <NumeroSerieProdutoSearchDialog
        open={searchOpen}
