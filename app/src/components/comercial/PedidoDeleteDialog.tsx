...
import { STATUS_LABELS } from '@/types/comercial/pedido.types';
...
export function PedidoDeleteDialog({ open, onOpenChange, pedido, onConfirm }: PedidoDeleteDialogProps) {
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <div className="rounded-lg border bg-muted/50 p-4 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Status: {STATUS_LABELS[pedido.status] || pedido.status}</p>
        </div>
        <AlertDialogDescription className="text-destructive">⚠️ Esta ação não pode ser desfeita. Os itens do pedido também serão excluídos.</AlertDialogDescription>
        <AlertDialogFooter>
