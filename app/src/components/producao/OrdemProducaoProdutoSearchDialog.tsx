...
export function OrdemProducaoProdutoSearchDialog({
  open,
  onOpenChange,
  produtos,
  onSelect,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-5xl">
        ...
        <div className="h-84 overflow-x-auto overflow-y-hidden">
          <DataGrid<Produto>
            tabId="op-produto-search-dialog"
            storageId="op-produto-search-dialog"
            columns={columns}
            data={filteredData}
            onSelect={(item) => setSelectedItem(item)}
            onActivate={handleActivate}
            activateOnDoubleClick
            emptyTitle="Nenhum produto fabricado encontrado"
          />
        </div>
