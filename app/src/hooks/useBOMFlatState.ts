 ...
 * Agora usa dados da API real (bomFlat do store) em vez de mock.
 */
...
const initialFilters: Filters = {
  codigoPaiExpressao: '', codigoPaiContem: '', codigoPaiComeca: '',
  codigoPaiNaoComeca: '', codigoPaiTermina: '', codigoPaiNaoTermina: '', codigoPaiNaoContem: '',
  descPaiExpressao: '', descPaiContem: '', descPaiComeca: '',
  descPaiNaoComeca: '', descPaiTermina: '', descPaiNaoTermina: '', descPaiNaoContem: '',
  qtdeMin: '', qtdeMax: '',
  codigoFilhoExpressao: '', codigoFilhoContem: '', codigoFilhoComeca: '',
  codigoFilhoNaoComeca: '', codigoFilhoTermina: '', codigoFilhoNaoTermina: '', codigoFilhoNaoContem: '',
  descFilhoExpressao: '', descFilhoContem: '', descFilhoComeca: '',
  descFilhoNaoComeca: '', descFilhoTermina: '', descFilhoNaoTermina: '', descFilhoNaoContem: '',
  unidade: '',
...
function matchText(
  value: string,
  expressao: string, contem: string, comeca: string,
  naoComeca: string, termina: string, naoTermina: string, naoContem: string,
) {
...
export function useBOMFlatState({ clearFiltersFlag, onOpenPai, selectedPai }: UseBOMFlatStateProps) {
  // Dados da API real (não mais mock)
  ...
  // Transforma bomFlat (API) nas rows que a tabela espera
  const rows = useMemo(() => {
    const base = bomFlat.map((item) => ({
      codigoPai: item.produtoPaiCodigo || '',
      descPai: item.produtoPaiDescricao || '',
      codigoFilho: item.produtoFilhoCodigo || '',
      descFilho: item.produtoFilhoDescricao || '',
      qtde: item.quantidade,
      ordem: String(item.posicao),
      unidade: item.produtoFilhoUnidade || 'UN',
      possuiDesenho: item.produtoFilhoTemDocumento || false,
    }));

    const filtered = base.filter((x) => {
      if (!matchText(x.codigoPai, filters.codigoPaiExpressao, filters.codigoPaiContem, filters.codigoPaiComeca, filters.codigoPaiNaoComeca, filters.codigoPaiTermina, filters.codigoPaiNaoTermina, filters.codigoPaiNaoContem)) return false;
      if (!matchText(x.descPai, filters.descPaiExpressao, filters.descPaiContem, filters.descPaiComeca, filters.descPaiNaoComeca, filters.descPaiTermina, filters.descPaiNaoTermina, filters.descPaiNaoContem)) return false;
      if (!matchText(x.codigoFilho, filters.codigoFilhoExpressao, filters.codigoFilhoContem, filters.codigoFilhoComeca, filters.codigoFilhoNaoComeca, filters.codigoFilhoTermina, filters.codigoFilhoNaoTermina, filters.codigoFilhoNaoContem)) return false;
      if (!matchText(x.descFilho, filters.descFilhoExpressao, filters.descFilhoContem, filters.descFilhoComeca, filters.descFilhoNaoComeca, filters.descFilhoTermina, filters.descFilhoNaoTermina, filters.descFilhoNaoContem)) return false;
  ...
  }, [bomFlat, filters, sortField, sortOrder]);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const typing = ae?.tagName === 'INPUT' || ae?.tagName === 'TEXTAREA' || ae?.getAttribute('contenteditable') === 'true';
